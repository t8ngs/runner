...
class Validator {
  /**
   * Ensures the t8ngs is configured. Otherwise raises an exception
   */
  ...
  /**
   * Ensures the t8ngs is in planning phase
   */
  ensureIsInPlanningPhase(phase: 'idle' | 'planning' | 'executing') {
    if (phase !== 'planning') {
      throw new Error(
        `Cannot import t8ngs test file directly. It must be imported by calling the "t8ngs.run" method`
      )
    }
  }
